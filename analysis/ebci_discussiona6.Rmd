---
title: "ebci disucssion simulation"
author: "Dongyue Xie"
date: "2022-02-09"
...
2. Empirikos Gaussian location mixture $LN([-6:0.05:6],0.25)$;
...
5. NPMLE, by using $LN([-6:0.02:6],0)$.

For the two simulation settings, we generate effects $\mu$ from 

1. $G_{spiky} = 0.4N(0,0.25^2)+0.2N(0,0.5^2)+0.2N(0,1^2)+0.2N(0,2^2)$;
...
for(i in 1:nreps){
  ...
  z = read.csv(paste('output/ebci_discussion/Z0_a6/Zs_rep',i,'.csv',sep=''))$Column1
...
saveRDS(list(lfsr0=lfsr0,lfsr_true=lfsr_true,
        ...
        file='output/ebci_discussion/ash_Z0_a6.rds')
```
...
```{r}
ploter = function(method,ebci,ash_lfsr,lfsr_true,idx,target,color = rgb(0, 0, 0.8,0.4),ylim=c(0,0.5)){
...
pdf(file = "output/ebci_discussion/spiky0_a6_51.pdf",  
    width = 3, 
    height = 15) 
...
No pointmass at 0 when generating data
...
for(i in 1:nreps){
  ...
  z = read.csv(paste('output/ebci_discussion/Z/Zs_rep',i,'.csv',sep=''))$Column1
  for(j in 1:length(nullweights)){
    ghat = ash(z,s=1,mixcompdist = 'normal',pointmass = TRUE, optmethod='mixSQP',
               prior = 'nullbiased',mixsd = mixsd,nullweight=nullweights[j])$fitted_g
    res = ash(target,s=1,g=ghat,fixg=TRUE)
...
saveRDS(list(lfsr=lfsr,lfsr0=lfsr0,lfsr_true=lfsr_true,
             g_true=g_true,nullweights=nullweights),
        file='output/ebci_discussion/ash_Z_a6.rds')
```
...
pdf(file = "output/ebci_discussion/spiky_a6_51.pdf",  
    width = 3, 
    height = 15) 

par(mfrow=c(5,1))
for(method in mtd_list){
  ploter(method,ebci,ash_lfsr,lfsr_true,idx=1:11,target=seq(-5,-3,by=0.2),ylim=c(0,0.3))
}
