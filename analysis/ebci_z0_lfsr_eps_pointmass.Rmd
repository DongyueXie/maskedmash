---
title: "Generate z scores from pointmass prior, Empirikos eps trick"
author: "Dongyue Xie"
date: "2022-02-07"
...
Nikos suggested a [trick](https://github.com/nignatiadis/Empirikos.jl/issues/11) to include pointmass, basically to use $p(\mu\geq -eps|Z=z)$ where $eps = 2.2\times 10^{-16}$. Now i tried this approach in lfsr confidence interval construction. 

```{r}
ebci = read.csv('output/ebci/lfsr_no_point_mass/spiky_pointmass_lfsr_eps_pointmass.csv')
head(ebci)
unique(ebci$method)
```
...
for(i in 1:nreps){
  ...
  z = read.csv(paste('output/ebci/lfsr_no_point_mass/Z0lfsr_eps/Zs_rep',i,'.csv',sep=''))$Column1
...
saveRDS(list(lfsr=lfsr,lfsr_true=lfsr_true,g_true=g_true,nullweights=nullweights),
        file='output/ebci/ash_lfsr_eps_pointmass.rds')
```

```{r}
ash_res = readRDS('output/ebci/ash_lfsr_eps_pointmass.rds')
