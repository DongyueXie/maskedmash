...
```{r}
ebci = read.csv('output/ebci/lfdr/lfdr0_eps/spiky0_lfdr0_eps_scale.csv')
```
...
lfdr_true = ash(target,s=1,g=g_true,fixg=TRUE)$result$lfdr
...
for(i in 1:nreps){
  ...
  z = read.csv(paste('output/ebci/lfdr/lfdr0_eps/Z0/Zs_rep',i,'.csv',sep=''))$Column1
...
saveRDS(list(lfdr=lfdr,lfdr_true=lfdr_true,g_true=g_true,nullweights=nullweights),
        file='output/ebci/lfdr/lfdr0_eps/ash_lfdr.rds')
```

```{r}
ash_res = readRDS('output/ebci/lfdr/lfdr0_eps/ash_lfdr.rds')
lfdr_mean = apply(ash_res$lfdr,3,colMeans)
colnames(lfdr_mean) = ash_res$nullweights
lfdr_true = ash_res$lfdr_true
```
...
plot_res = function(method,color,idx,ylim=c(0,1)){
  ...
  plot(target[idx],lfdr_true[idx],type='l',xlab='z',
  ...
  lines(target[idx],lfdr_true[idx])
  #lines(out$t,out$lower_mean,lty=2)
  #lines(out$t,out$upper_mean,lty=2)
  for(j in 1:3){
    lines(target[idx],lfdr_mean[idx,j],lty=j+1)
  }
